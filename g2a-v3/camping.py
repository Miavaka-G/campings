...
class AnnonceCamping(Scraping):
    def __init__(self, in_background:bool=True) -> None:
    ...
    def get_link_data(self) -> tuple:
        ...
        #MAJ le 19 08 2025 (car dans la barre d'adresse camping a changé en "created" tous les paramètres mais ne les affichent plus)
        url = self.url
        # input(f"voici l'url brute fa tsy chargé fa tsy poinsa ilay chargé => {self.url}")
        ...
        # input(f"jereo fa misy ny {checkin_date} sy ny {checkout_date}")
...
class CampingInitializer(Scraping):
    ...
    def extract(self) -> None:
        try:
            ...
            results = soupe.find('div', {'id': 'results'}).find_all('article', {'class': 'search-result'}) \
                if soupe.find('div', {'id': 'results'}).find_all('article', {'class': 'search-result'}) else []
